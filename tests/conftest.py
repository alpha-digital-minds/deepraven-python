...
from deepraven import DeepRaven, AsyncDeepRaven, DefaultAioHttpClient
from deepraven._utils import is_dict
...
logging.getLogger("deepraven").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[DeepRaven]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with DeepRaven(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncDeepRaven]:
